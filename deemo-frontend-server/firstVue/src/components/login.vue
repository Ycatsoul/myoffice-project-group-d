...
<div>
  <el-form :rules="rules" class="login-container" label-position="left" label-width="0px" v-loading="loading">
    ...
    <el-checkbox class="login_remember" v-model="checked" label-position="left">记住账号</el-checkbox>
  ...
  export default{
    ...
    methods:{
       submitClick() {
        var _this = this;
        this.loading = true;
        this.postRequestLogin('/login', {username: this.loginForm.username, password: this.loginForm.password}).then(resp=> {
          _this.loading = false;
...
            this.initRoutesStore(routes)
            ...
            if(this.$store.status.user.roleName === "ROLE_HR"){
               _this.$router.replace({path: path == '/' || path == undefined ? '/home' : path});
            } else if (this.$store.status.user.roleName === "ROLE_USER"){
               _this.$router.replace({path: path == '/' || path == undefined ? '/check/employeeCheck' : path});
            } 
          }
        });
      },
